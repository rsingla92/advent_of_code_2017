...
TEST_FILENAME = 'day_5_test.txt'
MY_FILENAME = 'day_5_input.txt'


def solve_maze(filename):
    with open(filename) as myFile:
        instructions = [int(i) for i in myFile]

    maze_length = len(instructions)
    cur_idx = 0
    next_idx = 0
    steps = 0

    while cur_idx < maze_length:
        steps += 1
        next_idx += instructions[cur_idx]
        instructions[cur_idx] += 1
        cur_idx = next_idx

    return steps

def solve_maze_2(filename):
    with open(filename) as myFile:
        instructions = [int(i) for i in myFile]
...
if __name__ == '__main__':
    print 'Steps to solve test maze: ' + str(solve_maze_2(TEST_FILENAME))
    print 'Steps to solve real maze: ' + str(solve_maze_2(MY_FILENAME))
